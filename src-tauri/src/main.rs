...
static PROVIDERS: Lazy<Vec<ProviderInfo>> = Lazy::new(|| {
  vec![
    ...
    ProviderInfo {
      ...
      static_models: vec!["claude-3-5-sonnet-latest".into(), "claude-3-5-haiku-latest".into()],
    },
    ProviderInfo {
      id: "google".into(),
      name: "Google".into(),
...
fn save_app_config(path: &Path, cfg: &AppConfig) -> Result<(), String> {
  let json = serde_json::to_string_pretty(cfg).map_err(|e| format!("serialize config error: {e}"))?;
...
fn resolve_within_workspace(workspace_path: &str, ruta: &str) -> Result<PathBuf, String> {
  ...
  let ws_norm = workspace.components().fold(PathBuf::new(), |mut acc, c| {
    acc.push(c.as_os_str());
    acc
  });

  if !normalize_path(&norm)
    .to_lowercase()
    .starts_with(&normalize_path(&ws_norm).to_lowercase())
  {
...
fn emit_live_status(app: &AppHandle, state: &LiveServerState) {
  let payload = LiveServerStatus {
    active: state.active,
    port: if state.active { Some(LIVE_SERVER_PORT) } else { None },
...
async fn ai_complete(provider: &str, model: &str, key: &str, messages: &[ChatRole]) -> Result<String, String> {
  ...
  match provider {
    "anthropic" => {
      ...
      let body: Value = resp.json().await.map_err(|e| format!("anthropic parse failed: {e}"))?;
    ...
    "google" => {
      ...
      let status = resp.status();
      let body: Value = resp.json().await.map_err(|e| format!("google parse failed: {e}"))?;
      if !status.is_success() {
    ...
    _ => {
      ...
      let status = resp.status();
      let body: Value = resp.json().await.map_err(|e| format!("provider parse failed: {e}"))?;
      if !status.is_success() {
...
#[tauri::command]
fn read_directory(dir_path: String) -> Result<Vec<TreeNode>, String> {
  Ok(read_directory_recursive(Path::new(&dir_path), 0))
}
...
#[tauri::command]
fn read_image_data_url(file_path: String) -> Result<HashMap<String, Value>, String> {
  ...
  let data_url = format!("data:{};base64,{}", mime, general_purpose::STANDARD.encode(bytes.clone()));
  let mut out = HashMap::new();
  out.insert("dataUrl".to_string(), Value::String(data_url));
  out.insert("size".to_string(), Value::Number((bytes.len() as u64).into()));
  Ok(out)
}
...
#[tauri::command]
fn create_file(file_path: String) -> Result<bool, String> {
...
#[tauri::command]
fn delete_item(item_path: String) -> Result<bool, String> {
...
fn watch_workspace(
  ...
  let watch_path = PathBuf::from(dir_path.clone());
...
#[tauri::command]
fn get_last_workspace(app: AppHandle, state: State<'_, Mutex<WorkspaceState>>) -> Result<Option<String>, String> {
  let config_path = {
    let mut s = state.lock().map_err(|_| "workspace state lock failed")?;
    if s.config_path.is_none() {
      s.config_path = Some(app_config_path(&app)?);
...
#[tauri::command]
fn ui_get_language(app: AppHandle, state: State<'_, Mutex<WorkspaceState>>) -> Result<Option<String>, String> {
  let config_path = {
    let mut s = state.lock().map_err(|_| "workspace state lock failed")?;
    if s.config_path.is_none() {
      s.config_path = Some(app_config_path(&app)?);
...
fn terminal_create(
  ...
  let mut push_candidate = |program: &str, args: &[&str]| {
    ...
    let exists = shell_candidates.iter().any(|c| {
      c.program.eq_ignore_ascii_case(&program_owned) && c.args == args_owned
    });
    if !exists {
  ...
  #[cfg(windows)]
  {
    match requested.as_str() {
      ...
      "git-bash" => {
        ...
        push_candidate("C:\\Program Files\\Git\\usr\\bin\\bash.exe", &["--login", "-i"]);
    ...
    loop {
      match reader.read(&mut buf) {
        ...
        Ok(n) => {
          ...
          let _ = app_for_read.emit("terminal:data", json!({ "id": id_for_read, "data": data }));
        }
        Err(_) => break,
      }
    }
  ...
  let mut s = terminal_state.lock().map_err(|_| "terminal state lock failed")?;
  s.sessions.insert(
    id.clone(),
    TerminalSession {
      writer,
...
fn terminal_write(
  ...
  let mut s = terminal_state.lock().map_err(|_| "terminal state lock failed")?;
  let session = s.sessions.get_mut(&id).ok_or("terminal session not found")?;
  session
    .writer
...
fn live_server_start(
  ...
  {
    let mut s = live_state.lock().map_err(|_| "live server state lock failed")?;
    ...
    loop {
      ...
      match server.recv_timeout(Duration::from_millis(150)) {
        Ok(Some(request)) => {
          ...
          let target = if rel.is_empty() { html_clone.clone() } else { rel.to_string() };
          ...
          match fs::read(&full) {
            Ok(bytes) => {
              ...
              if let Ok(header) = tiny_http::Header::from_bytes("Content-Type", mime.to_string()) {
...
#[tauri::command]
fn live_server_status(live_state: State<'_, Mutex<LiveServerState>>) -> Result<LiveServerStatus, String> {
  let s = live_state.lock().map_err(|_| "live server state lock failed")?;
  Ok(LiveServerStatus {
    active: s.active,
    port: if s.active { Some(LIVE_SERVER_PORT) } else { None },
    root: s.root.clone(),
    html_file: s.html_file.clone(),
    url: s.url.clone(),
  })
...
#[tauri::command]
fn lsp_request(lsp_state: State<'_, Mutex<LspState>>, args: LspRequestArgs) -> Result<Value, String> {
...
#[tauri::command]
fn lsp_notification(lsp_state: State<'_, Mutex<LspState>>, args: LspNotificationArgs) -> Result<bool, String> {
  let s = lsp_state.lock().map_err(|_| "lsp state lock failed")?;
  let sess = s.session.as_ref().ok_or("lsp not started")?;
  let msg = json!({
    "jsonrpc": "2.0",
...
#[tauri::command]
fn ai_get_config(app: AppHandle, state: State<'_, Mutex<WorkspaceState>>) -> Result<AiConfig, String> {
  let config_path = {
    let mut s = state.lock().map_err(|_| "workspace state lock failed")?;
    if s.config_path.is_none() {
      s.config_path = Some(app_config_path(&app)?);
...
fn ai_set_active(
  ...
  let mut cfg = load_app_config(&config_path);
  cfg.active_provider = if provider.is_empty() { None } else { Some(provider) };
...
async fn ai_chat_stream(
  ...
  let config_path = {
    let mut s = workspace_state.lock().map_err(|_| "workspace state lock failed")?;
    if s.config_path.is_none() {
      s.config_path = Some(app_config_path(&app)?);
    }
    s.config_path.clone().ok_or("missing config path")?
...
fn forge_read_history(workspace_path: String) -> Result<Option<Vec<Value>>, String> {
  let history_path = PathBuf::from(&workspace_path).join(".forge").join("history.json");
  if !history_path.exists() {
    return Ok(None);
  }
  let raw = fs::read_to_string(history_path).map_err(|e| format!("read history failed: {e}"))?;
  let parsed = serde_json::from_str::<Vec<Value>>(&raw).map_err(|e| format!("parse history failed: {e}"))?;
  Ok(Some(parsed))
}
...
fn forge_write_history(workspace_path: String, messages: Vec<Value>) -> Result<bool, String> {
  ...
  let body = serde_json::to_string_pretty(&messages).map_err(|e| format!("serialize history failed: {e}"))?;
...
#[tauri::command]
fn agent_buscar_en_proyecto(workspace_path: String, texto: String) -> Result<AgentSearchResult, String> {
  ...
  for entry in WalkDir::new(&workspace).into_iter().filter_map(Result::ok) {
    ...
    if file_name == "node_modules" || file_name == "dist" || file_name == "target" || file_name == ".git" {
      continue;
    }
...
fn agent_init_context(workspace_path: String) -> Result<AgentInitResult, String> {
  ...
  for entry in WalkDir::new(&ws).max_depth(4).into_iter().filter_map(Result::ok) {
    let p = entry.path();
...
#[tauri::command]
fn agent_snapshot_folder(workspace_path: String, folder_path: String) -> Result<Vec<AgentSnapshotFile>, String> {
  ...
  for entry in WalkDir::new(&folder).max_depth(6).into_iter().filter_map(Result::ok) {
    ...
    if let Some(content) = read_text_file_safe(p) {
      let rel = p.strip_prefix(&ws).unwrap_or(p).to_string_lossy().to_string();
